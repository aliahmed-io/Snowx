...
async function sendReceipt(formData: FormData) {
    "use server";
    // Mock email sending
    console.log("Sending receipt for order:", formData.get("orderId"));
...
export default async function OrderDetailsPage({ params }: OrderDetailsPageProps) {
    ...
    const getAddress = (json: any) => {
        if (!json || typeof json !== 'object') return null;
        return json as { line1?: string, city?: string, state?: string, postal_code?: string, country?: string };
    };
                    ...
                    <div className="bg-[#0a1628] border border-snow-primary/20 rounded-xl p-6">
                        ...
                        <form action={updateStatus} className="space-y-3">
                            ...
                            <select
                                ...
                                className="w-full bg-snow-primary/10 border border-snow-primary/20 rounded-lg p-2.5 text-white focus:border-snow-accent/50 focus:outline-none"
                            >
                                {statuses.map((s) => (
                            ...
                            <button
                                type="submit"
                                className="w-full bg-snow-accent text-[#020817] py-2 rounded-lg font-bold hover:bg-snow-accent/90 transition-colors"
                            >
                                Update Status
                            </button>
                        </form>
                    ...
                    <div className="bg-[#0a1628] border border-snow-primary/20 rounded-xl p-6">
                        ...
                        <div className="flex items-center gap-3 mb-6">
                            {order.user.profileImage ? (
                                <img src={order.user.profileImage} alt="" className="w-10 h-10 rounded-full" />
                            ) : (
                                <div className="w-10 h-10 rounded-full bg-snow-accent/20" />
                            )}
                            <div>
